...
int ft_parsing_scene(t_scene *scene)
{
    ...
    ft_coord(0,60, 200,scene->light->pos);
...
int    ft_list_sphere(t_sphere **s)
{
    ...
    ft_coord(30,20, 100,ptn->next->origin);
    ...
    ft_coord(0,290, 100,ptn->next->next->origin);
        ...
        while (sphere != NULL)
            {
                ...
                if (intersection_sphere(sphere,scene->ray,pos,normal,&t_min) == 1)
                {
                    ...
                    secondaire = *list;
                    while (secondaire != NULL)
                    {  
                        if ((secondaire != sphere) && (ft_visibilite(secondaire, ray_reflect, &t_inter) == 1 )) //  intersection
                                {
                            ...
                            secondaire = secondaire->next;
                    }
                if (V == -1)
                    V = 1;
                intensity = (scene->light->i * ft_max(ft_scal_produce(l,normal),255) * V) / dist ;
                if (intensity < 0)
                    intensity = 0;
                if (intensity > 255)
                    intensity = 255;
