...
double ft_ombre_2(t_scene *scene, double dist, t_ray ray_reflect)
{
    ...
    while (ptn != NULL)
    { 
        if (ptn->type == 0 && (intersection_sphere((t_sphere *)ptn->object, &ray_reflect, NULL, &t_inter)== 0)) 
            if (t_inter *t_inter < dist)
                    return(0);
        if (ptn->type == 1 && (intersection_plan((t_plan *)ptn->object,&ray_reflect,NULL,&t_inter) == 0))
            if (t_inter *t_inter < dist)
                    return(0);
        if (ptn->type == 2 && (intersection_square((t_square *)ptn->object, &ray_reflect,NULL,&t_inter) == 0))
            if (t_inter *t_inter < dist)
                    return(0);
